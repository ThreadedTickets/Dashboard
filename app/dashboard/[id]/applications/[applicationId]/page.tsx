...
export async function generateMetadata({
  ...
  return {
    ...
    } | Applications - Editing | Threaded Dashboard`,
  };
}
...
export default async function GuildDashboardPage({ params }: PageProps) {
  ...
  const { id: guildId, applicationId } = await params;
  const guilds = await getCachedGuilds(session.user.id, session.accessToken!);
  const focusedGuild = guilds.find((g) => g.id === guildId);
  ...
  const application = applications.find((m: any) => m._id === applicationId);
  if (!application) return <ThreadedNotInServer serverId={guildId} />;
    ...
    <div className="max-w-8xl mx-auto p-6 grid md:grid-cols-[1fr_5fr] grid-cols-1">
      ...
      <div className="p-4">
        <div className="flex justify-between">
          ...
          <a className="my-auto" href={`/dashboard/${guildId}/applications`}>
            <Button text="Back to all applications (save)" />
          </a>
        </div>
      </div>
    </div>
